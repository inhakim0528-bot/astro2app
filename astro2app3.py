...
with tab1:
    ...
    with col1_img:
        demo_alpha = st.slider("최대이각 α 조절 (도)", min_value=10.0, max_value=50.0, value=28.0, step=0.1, key="inner_slider")
        ...
        ax_inner.plot(sun_x, sun_y, 'yo', markersize=15, label='Sun')
        ax_inner.plot(earth_x, earth_y, 'bo', markersize=10, label='Earth')
        ax_inner.plot(planet_x, planet_y, 'ro', markersize=8, label='Inner Planet')
...
with tab2:
    ...
    * 구한 $\\alpha$를 측정된 이각 $\\epsilon$, $\\epsilon'$에서 빼어 $\\triangle EPE'$를 풀고, 최종적으로 코사인 법칙을 사용해 거리 $r$을 결정합니다[cite: 1].
    ...
    with col2_text:
        # 논문의 화성 예시 값(n=42.89)을 기본값으로 제공
        st.markdown("**[관측 파라미터 조절]**")
        n_val = st.slider("두 관측 시기 사이 지구의 궤도 각도 차이 n (도)", min_value=10.0, max_value=90.0, value=42.89, step=0.01)
        eps1 = st.slider("첫 번째 이각 ε' (도)", min_value=90.0, max_value=180.0, value=127.0, step=0.1)
        eps2 = st.slider("두 번째 이각 ε (도)", min_value=90.0, max_value=180.0, value=127.0, step=0.1)
...
with tab3:
    ...
    if len(st.session_state.planet_data) == 0:
        ...
        if len(st.session_state.planet_data) >= 2:
            ...
            st.success(f"🎉 **분석 결과:** Log-Log 그래프의 기울기가 **약 {m:.2f}**로 도출되었습니다. "
                       f"이상적인 기울기 1.5에 가깝다면, 수집된 거리가 공전주기와 조화의 법칙을 이룸을 성공적으로 증명한 것입니다!")
        else:
            st.info("회귀선과 그래프를 완벽히 분석하려면 최소 2개 이상의 행성 데이터가 필요합니다.")
